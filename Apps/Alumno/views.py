...
def inicio_primaria(request):
    ...
    if query:
        ...
        # Paginación
        paginator = Paginator(estudiantes_primaria, 5)  # 5 estudiantes por página
        page_number = request.GET.get('page')
        page_obj = paginator.get_page(page_number)
    ...
    context = {
        ...
        'total_estudiantes_primaria': estudiantes_primaria.count(),
        'total_estudiantes_secundaria': estudiantes_secundaria.count(),
        'total_estudiantes': estudiantes_primaria.count() + estudiantes_secundaria.count(),
        'query': query,
    }
...
def alumno_new(request):
    if request.method == 'POST':
        ...
        if alumno_form.is_valid():
            ...
            tipo_de_media = alumno_form.cleaned_data.pop('tipo_de_media', "")
            ...
            if tipo_alumno == 'primaria':
                ...
                alumno = EstudianteSecundaria.crear_estudiante(**cleaned_data, media=media, tipo_de_media=tipo_de_media)
            alumno.save()
            return redirect('inicio')
    else:
        alumno_form = AlumnoForm()
    return render(request, 'Alumno/alumno_form.html', {'alumno_form': alumno_form, 'next': request.GET.get('next', 'inicio'), 'url_name': 'alumno_new'})

@login_required
def alumno_update(request, etapa, id):
    next_url = request.GET.get('next', 'inicio')
    if etapa == 'primaria':
        alumno = get_object_or_404(EstudiantePrimaria, id=id)
    else:
        alumno = get_object_or_404(EstudianteSecundaria, id=id)
    if request.method == 'POST':
        ...
        alumno_form.fields['tipo_alumno'].initial = etapa
    return render(request, 'Alumno/alumno_form.html', {'alumno_form': alumno_form, 'next': next_url, 'url_name': 'alumno_update'})
